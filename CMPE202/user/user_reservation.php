  ...
  <head>
    ...
    <script>
      ...
      function renderReservationTable(data) {
        ...
        if (data["status"] == "success" &&
          ...
          for (var i = data["data"].length - 1; i >= 0; i--) {
            console.log(i);
            console.log(data["data"][i]["ORDER_STATUS"]);
            if (data["data"][i]["ORDER_STATUS"] == "In Progress" ||
                data["data"][i]["ORDER_STATUS"] == "Picked Up") {
              current_order_index = i;
              break;
            }
          }
          console.log("cur = " + current_order_index);

          if (current_order_index != -1) {
            var reservationResponse = data["data"][current_order_index];
      ...
      function renderHistoryTable(data) {
        if (data["status"] == "success" &&
            !jQuery.isEmptyObject(data["data"])) {
          var historyResponse = data["data"];

          var trHTML = '';
          $.each(historyResponse, function (i, item) {
            console.log(item)
            trHTML += '<tr><td>' + item["ORDER_ID"] + '</td><td>' + item["ORDER_STATUS"] + '</td><td>' + item["PRICE"] + '</td></tr>';
          });
      ...
      function sendReservationRequest() {
        ...
        $.ajax({
            ...
            success: function(data) {
              ...
              if (data["status"] == "success") {
                console.log(data["status"]);
                var current_order_index = renderReservationTable(data);
                console.log(data["data"])
                if (current_order_index != -1) {
                   var currentOrder = data["data"][current_order_index];
                   currentOrderId = currentOrder["ORDER_ID"];
                   sessionStorage.setItem("current_order_id", currentOrderId.toString());
                   console.log(sessionStorage.getItem("current_order_id"));
                }
              } else {
